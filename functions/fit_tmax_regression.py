...
def fit_tmax_regression(folder: str, input1: str, output1: str) -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if df.empty:
            raise RuntimeError("No valid TMAX observations found in the input CSV")

        df["day_of_year"] = df["date"].dt.dayofyear
        x = df["day_of_year"].values.astype(float)
        y = df["TMAX"].values.astype(float)

        faasr_log(f"Fitting linear regression on {len(x)} TMAX observations")
