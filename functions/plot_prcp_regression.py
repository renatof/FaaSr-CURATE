...
def plot_prcp_regression(folder: str, input1: str, input2: str, output1: str) -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if df.empty:
            raise RuntimeError("No valid PRCP observations found in current-year weather CSV")
        ...
        faasr_log(f"Plotting {len(x)} PRCP observations with regression line")
        ...
        ax.scatter(x, y, s=18, alpha=0.6, color="steelblue", label="Observed PRCP")
        ax.plot(x_line, y_line, color="tomato", linewidth=2, label=f"Regression fit\n{eq_label}")

        ax.set_title("Corvallis OR — Current Year PRCP with Linear Regression Fit")
    ...
    faasr_log("plot_prcp_regression complete")
