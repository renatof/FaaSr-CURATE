...
def plot_min_temperature(folder: str, input1: str, input2: str, output1: str) -> None:
    # ── Load inputs ───────────────────────────────────────────────────────────
    ...
    ax.fill_between(
        ...
        alpha=0.3,
        color="#1f77b4",
        label=f"_{current_year} TMIN fill",
    )
    ax.plot(
        cy_df["date"],
        cy_df["TMIN"],
        color="#1f77b4",
        linewidth=1.5,
        label=f"{current_year} Daily Min Temperature",
    )
    ax.plot(
        avg_dates,
        avg_df["TMIN_avg"],
        ...
        linewidth=1.8,
        linestyle="--",
        label="10-Year Daily Average Min Temperature",
    )

    ax.set_title(
        "Corvallis, OR — Daily Minimum Temperature: Current Year vs. 10-Year Average",
        fontsize=13,
        pad=12,
    )
    ax.set_xlabel("Date", fontsize=11)
    ax.set_ylabel("Minimum Temperature (°F)", fontsize=11)
    ax.xaxis.set_major_locator(mdates.MonthLocator())
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%b"))
    # suppress the fill_between proxy from the legend
    ...
    faasr_log(f"Min temperature comparison plot saved as {output1}")
